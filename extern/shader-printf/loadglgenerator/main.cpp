...
int main(int argc, char* argv[])
{
    ...
    while (std::getline(glext, line))
    {
        ...
        for (std::string& str : extensions)
            if (std::string::npos != line.find("#ifndef GL_ARB_" + str) || std::string::npos != line.find("#ifndef GL_EXT_" + str) ||
                std::string::npos != line.find("#ifndef GL_KHR_" + str) || std::string::npos != line.find("#ifndef GL_AMD_" + str))
            {
                ...
                if (std::string::npos != line.find("#ifdef GL_GLEXT_PROTOTYPES"))
                {
                    ...
                    while (std::getline(glext, line) && std::string::npos == line.find("#endif"))
                    {
                        ...
                        if (funcnames.count(stripfunc) == 0)
                        {
                            ...
                            load_debug << "\tif(!" << stripfunc
                                       << ") { MessageBox(0, \"Couldn't load OpenGL " + cur_version + ", function \\\"" + func +
                                          "\\\" is missing.\", \"OpenGL function missing\" , MB_OK); result = -1; }" << std::endl << std::endl;
                        }
                    }
                    --inside;
                }
    ...
    load_check << "\t\t\t" << "false ) {" << std::endl
               << "\t\tMessageBox(0, \"Couldn't load OpenGL " + std::to_string(maxver / 10) + "." + std::to_string(maxver % 10);
