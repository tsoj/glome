...
namespace gli
{
    ...
    inline sampler_cube<T, P>::sampler_cube(texture_cube const& Texture, gli::wrap Wrap, filter Mip, filter Min, texel_type const& BorderColor)
        : sampler(Wrap, Texture.levels() > 1 ? Mip : FILTER_NEAREST, Min), Texture(Texture),
          Convert(detail::convert<texture_cube, T, P>::call(this->Texture.format())), BorderColor(BorderColor), Filter(
    ...
    sampler_cube<T, P>::texel_fetch(extent_type const& TexelCoord, size_type Face, size_type Level) const
    {
    ...
    template<typename T, qualifier P>
    inline typename sampler_cube<T, P>::texel_type
    sampler_cube<T, P>::texture_lod(normalized_type const& SampleCoord, size_type Face, level_type Level) const
    {
        GLI_ASSERT(!this->Texture.empty());
    ...
    {
        this->generate_mipmaps(this->Texture.base_face(), this->Texture.max_face(), this->Texture.base_level(), this->Texture.max_level(),
                               Minification);
    }

    template<typename T, qualifier P>
    inline void
    sampler_cube<T, P>::generate_mipmaps(size_type BaseFace, size_type MaxFace, size_type BaseLevel, size_type MaxLevel, filter Minification)
    {
        GLI_ASSERT(!this->Texture.empty());
