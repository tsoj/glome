...
namespace gli
{
    ...
    inline bool save_dds(texture const& Texture, std::vector<char>& Memory)
    {
        ...
        bool const RequireDX10Header =
            DXFormat.D3DFormat == dx::D3DFMT_GLI1 || DXFormat.D3DFormat == dx::D3DFMT_DX10 || is_target_array(Texture.target()) ||
            is_target_1d(Texture.target());

        Memory.resize(
            Texture.size() + sizeof(detail::FOURCC_DDS) + sizeof(detail::dds_header) + (RequireDX10Header ? sizeof(detail::dds_header10) : 0));
