...
namespace level
{
    int test_format(gli::format Format)
    {
        ...
        for (gli::texture2d::size_type Level = 0; Level < Texture.levels(); ++Level)
        {
            ...
            gli::image::extent_type const& Extent0 = glm::max(
                Texture[0].extent() >> gli::image::extent_type(static_cast<gli::image::extent_type::value_type>(Level)), gli::image::extent_type(1));
