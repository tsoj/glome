...
int test_sub_copy()
{
    ...
    Destination.copy(Source, 0, 0, 0, gli::texture::extent_type(1, 1, 0), 0, 0, 0, gli::texture::extent_type(1, 1, 0),
                     gli::texture::extent_type(2, 2, 1));
    for (gli::size_t IndexY = 1; IndexY < 3; ++IndexY)
        for (gli::size_t IndexX = 1; IndexX < 3; ++IndexX)
            Error += Source.load<gli::u8>(gli::texture2d::extent_type(IndexX, IndexY), 0) ==
                     Destination.load<gli::u8>(gli::texture2d::extent_type(IndexX, IndexY), 0) ? 0 : 1;
